...
const GENRES = [
  { name: "Haryanvi", query: "haryanvi song official video 2024", color: "from-orange-500 to-red-500" },
  { name: "Bollywood", query: "bollywood song official video 2024", color: "from-purple-500 to-pink-500" },
  { name: "Punjabi", query: "punjabi song official video 2024", color: "from-green-500 to-teal-500" },
  { name: "Rajasthani", query: "rajasthani song official video", color: "from-pink-500 to-rose-500" },
  { name: "Hip Hop", query: "hip hop song official video 2024", color: "from-blue-500 to-indigo-500" },
  { name: "Lo-Fi", query: "lofi song video 2024", color: "from-cyan-500 to-blue-500" },
  { name: "Trending", query: "top trending song 2024 official", color: "from-yellow-500 to-orange-500" },
  { name: "Mix", query: "english song official video popular 2024", color: "from-yellow-500 to-purple-500" },
];
  ...
  const SongItem = ({ song, allSongs, showActions = true }: { song: Song; allSongs: Song[]; showActions?: boolean }) => {
    ...
    return (
      ...
      >
        ...
        <span className="text-xs md:text-sm text-gray-400 hidden md:block">{song.duration ? `${Math.floor(song.duration / 60)}:${String(song.duration % 60).padStart(2, '0')}` : '--:--'}</span>
        ...
        {showActions && (
          <div className="flex items-center gap-1 md:gap-2">
            ...
            <button onClick={(e) => handleAddToQueue(e, song)} className="p-1 md:p-2 text-gray-400 hover:text-indigo-400 opacity-0 group-hover:opacity-100">
              <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              </svg>
            </button>
            <button onClick={(e) => handlePlayNext(e, song)} className="p-1 md:p-2 text-gray-400 hover:text-indigo-400 opacity-0 group-hover:opacity-100">
              <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 10l7-7m0 0l7 7m-7-7v18" />
              </svg>
            </button>
            <button onClick={(e) => downloadSong(e, song)} className="p-1 md:p-2 text-gray-400 hover:text-indigo-400 opacity-0 group-hover:opacity-100">
              <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {showSearch && searchQuery ? (
          ...
          <div className="space-y-4">
            ...
            {state.playlists.length === 0 ? (
              <p className="text-gray-400 text-sm">No playlists yet. Create one from the Local Songs tab!</p>
            ) : (
              <div className="space-y-2">
